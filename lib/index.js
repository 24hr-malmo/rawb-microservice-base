...
const start = async (options) => {
    ...
    server.on('restifyError', function(req, res, err, callback) {
        ...
        if (options.logger) {
            ...
            options.logger.error('Restify reported an error: %s %s %s', err.message, status, err.stack, { requestId });
        }
        res.status(status);
        let response = {
            success: false,
    ...
    server.use(async (req, res, next) => {
        ...
        try {
            ...
            if (options.logger) {
                let requestId = req.headers['x-request-id'];
                options.logger.error('Error: %s %s %s', err.message, status, err.stack, { requestId });
            }
            res.status(status);
